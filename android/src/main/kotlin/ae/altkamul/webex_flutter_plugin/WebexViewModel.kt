                ...
                result.error?.let { error ->
                    when(error.errorCode){
                        WebexError.ErrorCode.INVALID_API_ERROR.code -> {
                            _callingLiveData.postValue(
                                WebexRepository.CallLiveData(
                                    WebexRepository.CallEvent.WrongApiCalled, null, null, result.error?.errorMessage))
                        }
                        else -> {
                            _callingLiveData.postValue(
                                WebexRepository.CallLiveData(
...
/*
    ...
    fun refreshCaptcha() {
        webex.phone.refreshMeetingCaptcha() {
            _callingLiveData.postValue(
                WebexRepository.CallLiveData(
                    WebexRepository.CallEvent.CaptchaRequired, null, it.data))
        }
    }
        ...
        call.answer(mediaOption, CompletionHandler { result ->
            if (result.isSuccessful) {
                result.data.let {
                    _callingLiveData.postValue(
                        WebexRepository.CallLiveData(
                            WebexRepository.CallEvent.AnswerCompleted, call))
                }
            } else {
                _callingLiveData.postValue(
                    WebexRepository.CallLiveData(
                        WebexRepository.CallEvent.AnswerFailed, null, null, result.error?.errorMessage))
            }
        })
    }
*/
            ...
            if (result.isSuccessful) {
                ...
                result.data?.let {
                    ...
                    _startAssociationLiveData.postValue(
                        WebexRepository.CallLiveData(
                            WebexRepository.CallEvent.AssociationCallCompleted, it))
                }
            } else {
                Log.d(tag, "startAssociatedCall Lambda isSuccessful 5")
                _startAssociationLiveData.postValue(
                    WebexRepository.CallLiveData(
                        WebexRepository.CallEvent.AssociationCallFailed, null, null, result.error?.errorMessage))
...
//        repository.removeIncomingCallListener("viewmodel"+this)
  ...
  /*  fun printObservers(writer : PrintWriter) {
    ...
    fun startAudioDump() {
        getCall(currentCallId.orEmpty())?.startRecordingAudioDump(WebexCallApp.applicationContext()) {
            if (it.isSuccessful) {
